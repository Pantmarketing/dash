...
app.post(
  "/api/login",
  zValidator("json", objectType({ password: stringType() })),
  async (c) => {
...
const DashboardSchema = objectType({
  name: stringType(),
  business_model: enumType([
    "lead_para_vendedor",
    "venda_direta",
    "lead_instagram",
    "lead_mensagens"
  ]),
  sheets_url: stringType().optional()
});
const DashboardDataSchema = objectType({
  date: stringType(),
  investment: numberType(),
  clicks: numberType(),
  leads: numberType(),
  conversations: numberType(),
  sales: numberType(),
  revenue: numberType(),
  page_views: numberType().default(0),
  checkouts: numberType().default(0),
  meetings: numberType().default(0),
  average_ticket: numberType().default(0),
  profile_clicks: numberType().default(0),
  followers: numberType().default(0),
  messages: numberType().default(0)
});
app.post(
  "/api/dashboards",
...
app.post(
  ...
  zValidator("json", arrayType(DashboardDataSchema)),
  async (c) => {
    const id = c.req.param("id");
    ...
    for (let i = 0; i < dataLines.length; i++) {
      ...
      if (values.length === 7) {
        ...
        if (values.length === 14) {
          ...
          leadsStr = values[4] || values[3];
          conversationsStr = values[5] || values[4];
          salesStr = values[values.length - 2];
          revenueStr = values[values.length - 1];
        }
      } else {
        leadsStr = values[3];
...
app.put(
  "/api/dashboards/:id/sheets-url",
  zValidator("json", objectType({ sheets_url: stringType() })),
  async (c) => {
    const id = c.req.param("id");
...
app.post("/api/dashboards/:id/import-sheets", async (c) => {
  ...
  try {
    ...
    let sheetsUrl = dashboard.sheets_url;
    if (!sheetsUrl) {
